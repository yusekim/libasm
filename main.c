...
void test_ft_strlen() {
    ...
    // Test Case 3: 긴 문자열
    {
        const char *str = "This is a very long string to test the behavior of ft_strlen with longer inputs.";
        size_t result = ft_strlen(str);
    ...
    // Test Case 6: 매우 짧은 문자열 (1글자)
    {
        const char *str = "A";
        size_t result = ft_strlen(str);

        printf("Test 5 - Single Character String:\n");
        printf("Input: \"%s\"\n", str);
        printf("Expected: %zu\n", strlen(str));
        printf("Result: %zu\n", result);
        printf("Pass: %s\n\n", (result == strlen(str)) ? "Yes" : "No");
